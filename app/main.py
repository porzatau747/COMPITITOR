...
@app.get("/health")
def health():
    return {"ok": True, "service": "Advice Content Radar"}
...
app.include_router(sources.router, dependencies=[Depends(require_admin_api_key_header)])
app.include_router(posts.router, dependencies=[Depends(require_admin_api_key_header)])
